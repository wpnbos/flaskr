...
IMAGE_FOLDER = "flaskr/static"
allowed_extensions = {"jpg", "png", "gif", "jpeg"}

def allowed_file(filename):
	return '.' in filename and filename.rsplit('.', 1)[1].lower() in allowed_extensions

def upload_img(user_id, image):
...
def index():
	db = get_db()
	posts = db.execute(
		"SELECT p.id, title, body, created, author_id, username, likes, image FROM post p JOIN user u ON p.author_id = u.id ORDER BY created DESC"
	).fetchall()
...
def create():
	if request.method == "POST":
		...
		if error is not None: 
			...
			if image: 
				print("iamge!!")
				image_path = upload_img(g.user["id"], image)
			db = get_db()
			db.execute(
				"INSERT INTO post (title, body, author_id, image) VALUES (?, ?, ?, ?)",
				(title, body, g.user["id"], image.filename)
			)
			db.commit()
